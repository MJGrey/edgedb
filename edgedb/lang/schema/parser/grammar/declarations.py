...
class DeclarationBase(Nonterm):
    ...
    def reduce_PropertyDeclaration(self, kid):
        self.val = kid.val
...
class Link(Nonterm):
    def _process_pointerspec(self, p: PointerSpec, linktype: type):
        constraints = []
        attributes = []
        policies = []
        properties = []
    ...
    def reduce_LINK_Spec(self, *kids):
        self.val = self._process_pointerspec(kids[1].val, esast.Link)

    def reduce_REQUIRED_LINK_Spec(self, *kids):
        link = self._process_pointerspec(kids[2].val, esast.Link)
        link.required = True
        self.val = link
